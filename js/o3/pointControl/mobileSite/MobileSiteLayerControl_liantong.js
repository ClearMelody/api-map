...
var MobileSiteLayerControl_liantong = function(){
	this.init.apply(this, arguments);
}

MobileSiteLayerControl_liantong.prototype ={
		map:null,
...
//			popupControl.showPopUpWin(this.tempContent,[feature.get("LON"),feature.get("LAT")]);
			...
			text+='   <span onclick="mobileSiteLayerControl.showSelectPopUp(this)" cname="'+feature.get("中文名")+'"  LAC="'+feature.get("LAC")+'" CINO="'+feature.get("CI")+'" lon="'+feature.get("经度")+'"  lat="'+feature.get("纬度")+'"  ';
...
//			var content = $(".list_box_1",$(obj)).html();
		...
		queryMobileByArcgis : function(LAC,CINO,curPage,pageSize){
			...
			'where=LAC+%3D+' + LAC + "+and+CINO+%3D+" + CINO + 
			...
			$.ajax({url: url, dataType: 'jsonp', success: function(response) {
				if (response.error) {
					...
					map.getView().setCenter([response.features[0].attributes.LON*1,response.features[0].attributes.LAT*1]);
					centerCrossEffect.startAnimate(response.features[0].attributes.LON*1,response.features[0].attributes.LAT*1);
					// dataProjection will be read from document
					//resultWinControl_Mobile.mobileResultWinByArcgis(response.features,curPage,pageSize);
				}
			}});
		},
		/*
		...
		CLASS_NAME : "MobileSiteLayerControl_liantong"
}