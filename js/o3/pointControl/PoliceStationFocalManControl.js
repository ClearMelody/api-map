var PoliceStationFocalManControl = function () {
    this.init.apply(this, arguments);
};
PoliceStationFocalManControl.prototype = {
	_PoliceStationFocalManControl : null,
   	baseMapLayerControl : null,
    popupId : null,
    pointArr : [],
    layerMark : "focalman",
    init: function (map,isFaceLayer) {
    	_PoliceStationFocalManControl = this;
    	_PoliceStationFocalManControl.initMustJsFile();
    	_PoliceStationFocalManControl.initMustHtml();
    	
        _PoliceStationFocalManControl.baseMapLayerControl=new BaseMapLayerControl({
            map : map,
            layerMark : _PoliceStationFocalManControl.layerMark,
            layerName : "重点人员",
            layerVisible : true,
            url : "getFocalmanOlClusterView.do?findAllFocalman=false",
            getSingleLayerMarker : _PoliceStationFocalManControl.getSingleLayerMarker,
            getClusterLayerMarker : _PoliceStationFocalManControl.getClusterLayerMarker,
            clusterLayerClick : _PoliceStationFocalManControl.clusterLayerClick,
            singleLayerClick : _PoliceStationFocalManControl.singleLayerClick,
            singleLayerDblClick : _PoliceStationFocalManControl.singleLayerDblClick
        });
    },
    //1.必须方法之单个点图层双击事件
    ...
    initMustHtml : function(){
    	...
    	html+='				</div>';
    	html+='		</div>';
    	html+='		<div target="dynamicContent" style="overflow: auto;"><!--滚动区域-->';
    ...
    initMustJsFile : function(){
        ...
        for (var i = 0; i < jsArray.length; i++) {
	        if (_PoliceStationFocalManControl._isIncludeJs(jsArray[i]) == false) {
    ...
    clusterLayerClick : function(coordinate,arr){
    	_PoliceStationFocalManControl.pointArr=arr;
		var lon=0;
		var lat=0;
		var id;
	    ...
	    $(".GkkListNav[focalmanid]").unbind("click").bind("click",function(e){
	    	...
	    	var focalman=_PoliceStationFocalManControl.getFocalmanById(focalmanid);
	    	setTimeout(function(){
    ...
    getFocalmanById : function(id){
    	var obj={};
    	var arr=_PoliceStationFocalManControl.pointArr;
    	if(arr){
	    	for(var i=0;i<arr.length;i++){
	    		if(arr[i].id==id){
	    			obj=arr[i];
    ...
    showLayer : function(visible){
        _PoliceStationFocalManControl.baseMapLayerControl.showLayer(_PoliceStationFocalManControl.layerMark,visible);
    },
    //7.必须方法之设置图层的显示与隐藏,固定写法
    _setLayerVisible: function () {
        _PoliceStationFocalManControl.baseMapLayerControl._setLayerVisible(_PoliceStationFocalManControl.layerMark);
    },
    CLASS_NAME: "PoliceStationFocalManControl"
};