/**
 * @(#)ThirdClassCaseControl.js
 * 
 * @description: 三类点案件关联管理
 * @author: 肖振亚 2016/3/2
 * @version: 1.0
...
var ThirdClassCaseControl = function(){
	this.init.apply(this, arguments);
}

ThirdClassCaseControl.prototype ={
	map : null,
...
getThirdClassPoint : function(){
	var _self = this;
	ZT.Utils.Ajax().request("getThirdCaseClassPoint.do",{
		data : "caseId=" + parent.caseId,
...
//					var cor = ol.proj.transform([contentTemp.lon + 0.0060,  contentTemp.lat - 0.0027], 'EPSG:4326', 'EPSG:3857');
				...
				if((contentTemp.viewAngle && contentTemp.viewAngle > 0) || (contentTemp.horizontalIncludedAngle && contentTemp.horizontalIncludedAngle > 0) ){
					drawControl.addFans([cor[0], cor[1]],radioNew,contentTemp.viewAngle*1,contentTemp.horizontalIncludedAngle*1,_self.thirdClassSource);
				}
			}
...
addPopUp : function(content,coordinate){
    ...
    obj.find("a[aliasid='videoMangeBtn']").click(function(){
		...
		openVideoManagerDialog(contentTemp);
    })
    //关联资源
    ...
    obj.find("a[aliasid='delPointBtn']").click(function(){
    	jQuery.dialog.confirm("确认删除三类点？", function(){
    		popupControl.closePopUpwin();
...
clear : function(){
	...
	CLASS_NAME : "ThirdClassCaseControl"
