...
(function(){
	...
	//openlayer3 获取多个多边形的字符串，如环形
	Utils.getManyPolyGonString = function(CoordinatesArr){
		if(!CoordinatesArr){
			return;
		}
		var extent ='';
		for(var j=0;j<CoordinatesArr.length;j++){
			var Coordinates = CoordinatesArr[j];
			extent += '(' ;
			for(var i=0;i < Coordinates.length;i++){
				extent += Coordinates[i].join(' ') + ', ';
			}		
			extent = extent.substr(0,extent.length - 2);
			extent += '), ';
		}
		extent = extent.substr(0,extent.length - 2);
		extent = 'POLYGON(' +  extent + ')';
	...
	Utils.transformPoint  = function (pnt) {
      //  var clientGISKind = window.gisAdapter.gisType;
        if (top.window.gisType=="OFFLINEGIS") {
	...
	//将离线地图坐标转换为常规地图坐标
	Utils.transformListsOffToESPG  = function (pntList) {
		if (top.window.gisType=="OFFLINEGIS") {
			var coords = [];
			var coord = [];
			for (i = 0; i < pntList.length; i++) {
                var tempcor = pntList[i];
                for(j=0;j<tempcor.length;j++){
                	var temp = tempcor[j];
                	var cor = ZT.Utils.transformOffToESPG(temp);
                	coord.push(cor);
                }
                coords.push(coord);
            }
	...
	Utils.getPolygonWkt = function (wkt) {
        ...
        for(var i= 0;i<pntList.length;i++){
        	var point = ZT.Utils.transformPoint(pntList[i]);
